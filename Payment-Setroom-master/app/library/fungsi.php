<?php  
	class oop{
		...
		function simpan($table,array $field){
			...
			$sql = rtrim($sql, ',');
			$jalan = mysqli_query($sql);
		}

		function tampil($table,$where,$cari){
			$sql = mysqli_query("SELECT * FROM $table $where $cari");
		...
		function edit($table,$where){
			...
			$jalan = mysqli_fetch_array(mysqli_query($sql));
			return $jalan;
		}

		function hapus($table,$where){
			$sql = mysqli_query("DELETE FROM $table WHERE $where");
			return $sql;
		}
		...
		function caridata($table){
			$sql = mysqli_fetch_array(mysqli_query("SELECT * FROM $table"));
			return $sql;
		}

		function cekdata($table){
			$sql = mysqli_num_rows(mysqli_query("SELECT * FROM $table"));
			return $sql;
		}		
		...
		function login($table,$username,$password,$alamat){
			@session_start();
			$sql = mysqli_query("SELECT * FROM $table WHERE username = '$username' AND password = '$password'");
